...
- コイン/路面タイルの見た目は仮(`BoxStaticMeshComponent` は色指定 API が無く赤単色)。専用モデル/色指定は今後の課題。
  取得 SE も Decision.wav 流用の仮。専用アセット導入時に `CoinComponentSystem.cpp` のパスを差し替える。
- 箱はバウンディング無しでフラスタムカリングされないため、路面タイル約 360 枚で 180fps→60-95fps に低下。
  タイルの実用上は 60fps を維持できているが、バウンディング付与かインスタンシング化で改善余地あり。
