...
namespace aq
{
	namespace graphics
	{
		...
		{
		public:
			static constexpr uint32_t MAX_RT = 8;
			...
			struct Key
			{
				...
				uint32_t                      rtCount = 1;
				DXGI_FORMAT                   rtFormats[MAX_RT] = { DXGI_FORMAT_R8G8B8A8_UNORM };
			...
			// コンピュート PSO (CS バイトコードをキーにキャッシュ)。
			ID3D12PipelineState* GetOrCreateCompute(ID3D12Device* device, ID3D12RootSignature* rootSig,
			                                        D3D12Shader* cs);
			void Release();
		...
		private:
			...
			std::unordered_map<const void*, ID3D12PipelineState*>  computeCache_;
		};
	}
}
