...
> 対象コミット: b3150d4 / 最終更新: 2026-09-08
...
| 数学 | DirectXMath を継続 | ヘッダオンリで clang/ARM64 NEON 対応。置換不要 |
...
| ファイル | 責務 | 変更 |
|---|---|---|
| `aqEngine/Platform/PlatformDefs.h` **(新規)** | `AQ_PLATFORM_*` の既定値決定(未定義 かつ `_WIN32` → `AQ_PLATFORM_WIN32`)と「ちょうど 1 つ」検証。`AQ_PLATFORM_DESKTOP`(WIN32 or MAC)等の派生マクロ | `aq.h` 先頭で include |
...
| `aqEngine/Platform/DebugOutput.h` **(新規)** + `DebugOutputWin32.cpp` / `DebugOutputMac.cpp` | `aq::debug::OutputString(const char*)`。Win32/UWP = `OutputDebugStringA`、Mac = `fputs(stderr)` | 新規 |
| `aqEngine/Platform/AlignedAlloc.h` **(新規)** | `aq::memory::AlignedAlloc(size, align)` / `AlignedFree`。Win32 = `_aligned_malloc`、Mac = `posix_memalign`。`Memory/HeapAllocator.h`・`StackAllocator.h` 等 13 箇所を置換 | 新規 |
...
| `aqEngine/Resource/ImageLoader.{h,cpp}` **(新規)** | `LoadImageFile(path) → ScratchImage`(DirectXTex 型は維持)。拡張子で DDS/TGA は DirectXTex、PNG/JPG は WIN32/UWP なら `LoadFromWICFile`、MAC なら `stb_image` → `Image` 構造体へ詰めて `ScratchImage::InitializeFromImage`。`Resource.cpp:1607-1625`・`Terrain/HeightmapChunk.cpp:50-133` の直呼びをこれに集約 |
...
- `aq.h`/`Utility.h` 分割、`#pragma comment(lib)` 撤去(vcxproj `AdditionalDependencies` へ)。
...
評価:
- [ ] 既存 `DirectX.sln`(MSVC)で Debug/Release/UWP がビルドでき、AquaDash が従来どおり動く(D3D12 / Vulkan 両構成)
- [ ] CMake → VS 生成で Engine/Game がビルドでき、同じ exe が起動する
- [ ] CMake → clang-cl 構成でエンジンと Game が**コンパイル・リンク**できる(実行は任意)
- [ ] `AQ_PLATFORM_*` を 2 つ定義すると `#error`、未定義なら WIN32 になる
- [ ] `grep "#pragma comment(lib"` が 0 件、`grep "!defined(AQ_PLATFORM_UWP)"` が意図した箇所のみ
