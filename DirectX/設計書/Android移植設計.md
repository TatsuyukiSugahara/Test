...
**本書のステータス: P3 / P4 / P5 / P7 まで完了(2026-09-13)。次は P6(性能とパッケージング)。**
P7(タッチ操作)は P4 の積み残しで、実機で UI を触れないと評価が進まないため P6 より先に片付けた。
...
### P4: 入力 — 完了(2026-09-13。デバッグ UI のタッチ操作は P7 へ)
...
### P7: タッチ操作の仕上げ — 完了(2026-09-13)

P4 で積み残した「デバッグ UI が指で操作できる」を回収する。**P6(性能)より先に着手する** —
実機で UI を直接触れないと、仮想パッドのレイアウトや操作感の評価そのものが進まないため。

#### 現状の何が問題か

タッチ自体は取れている。足りないのは**それをポインタとして扱う経路**。

| | 症状 | 原因 |
|---|---|---|
| ImGui | 見えているのに触れない | `Application.cpp` のプラットフォームバックエンド分岐が Win32 / Mac しか無く、Android は `#else` で `DisplaySize` と `DeltaTime` しか埋めていない |
| ゲーム UI | ボタンを直接タップしても反応しない | `UIInputSystem::UpdatePointer` はマウスしか見ておらず、Android の `DefaultMouseBackend` は `NullMouseBackend`。タイトルから進めているのは仮想パッドの A → `IsSubmit()` というフォーカス操作の経路だけ |

#### 方針: タッチをマウス抽象へ流す

| | |
|---|---|
| 決定 | タッチから「ポインタ」を合成し、`IMouseBackend` の実装として供給する |
| 理由 | `UIInputSystem` が無改修で動く。ImGui シムも「マウス状態を `io` へ渡すだけ」で済み、既にある `SuppressMouse(io.WantCaptureMouse)` の調停がそのまま効く。hover / click の判定を 2 経路に分けずに済む |
| 代償 | 「Android に物理マウスは無い」という現在の記述を書き換える。**物理マウスを想定するという意味ではなく、タッチを既存のポインタ経路へ載せるという意味**である点はコメントで明示する |

`imgui_impl_android` は使わない(Mac の `imgui_impl_osx` を採らなかったのと同じ判断)。
入力の入り口を `PlatformAndroid` の 1 本に保つほうが、取り合いの調停を 1 箇所で書ける。

#### 指の取り合い

1. **仮想パッドが使っている指はポインタにしない。** スティックを倒しながら裏の UI を
   誤クリックしないため。スティックは掴んだ id、ボタンは円内判定で除外する
2. 残りの指のうち 1 本目をカーソルとし、触れている間を左ボタン押下とする
3. ImGui がポインタを取ったフレームは既存の抑制で UI 側が止まる
   (`io.WantCaptureMouse` は前フレームの値。マウスと同じ 1 フレーム遅れで扱う)

#### 構成

| ファイル | 役割 |
|---|---|
| `HID/TouchMouseBackend.{h,cpp}` **(新規)** | `IMouseBackend` 実装。プラットフォーム非依存(iOS と共有)。**渡された `TouchState` の 1 本目をポインタにするだけ**で、仮想パッドとの取り合いは見ない(選り分けは `InputManager` が済ませる) |
| `HID/KeyboardMouseBackend.h` | Android の `DefaultMouseBackend` を `NullMouseBackend` → `TouchMouseBackend` へ |
| `HID/VirtualPadBackend.{h,cpp}` | 「この指は自分が使っている」を問える API を追加 |
| `HID/Input.{h,cpp}` | パッドが使っていない指だけの集合 `pointerTouch_` を毎フレーム構築し、マウスバックエンドへ渡す。更新順も「タッチ → キーボード → パッド → `pointerTouch_` → マウス」へ変える |
| `Platform/Common/ImGuiPointerInput.{h,cpp}` **(新規)** | マウス抽象の状態を ImGui へ流すシム。`MacImGui` と同じ構えだがプラットフォーム非依存 |
| `Core/Application.cpp` | ImGui の init / NewFrame / shutdown に分岐を追加。モバイルでのスケール設定 |

#### ImGui へ渡すときの注意

`aq::hid` の**自由関数(`IsMousePressed` 等)は `IsMouseSuppressed()` を見て false を返す**。
抑制は「ImGui が使っている間ゲーム側を止める」ための仕組みなので、ImGui シムがこれを使うと
**ImGui が入力を取った次のフレームから何も届かず、永久に触れなくなる**。
シム側は `InputManager::GetMousePtr()` から `Mouse` を取り、抑制を見ない生の判定を読むこと。

#### 指で使える尺度

2400x1080 ではメニューバーの高さが 10px 程度しかなく、入力を繋いでも実用にならない。
`ImGui::GetStyle().ScaleAllSizes()` と `io.FontGlobalScale` をモバイルで上げる。
倍率は実機で決める(初期値 2.0 から)。

**評価チェックリスト(P7)** — 実機確認 2026-09-13 / Pixel 7a・Android 16
- [x] Android ビルドがエラー 0・新規警告なし / Windows 3 構成が従来と同数(警告 54 件)
- [x] **ImGui のメニューとウィンドウが指で操作できる** — メニューバーのタップ → 項目選択 →
      ウィンドウ表示 → タイトルバーのドラッグ移動まで確認
- [x] 仮想パッドのスティックを操作しても、裏の UI / ImGui が誤反応しない —
      スティックを掴んだまま ImGui ウィンドウの上まで指を動かしても、ウィンドウは動かず
      誤クリックもしない(走行は継続)
- [x] 物理コントローラでの従来の操作(フォーカス移動 + A)が壊れていない —
      仮想 A でタイトル → ステージへ進めることを確認
- [x] **Windows のマウス操作が壊れていない** — 実行して ImGui のメニューがマウスで開くことを確認。
      183 FPS・正常終了。ImGui の拡大は Android だけに掛かっている
- [—] ゲーム UI のボタンが直接タップで反応する — **対象なし**。AquaDash のメニューは
      `UIButtonComponent` ではなく自前のカーソル方式(`AquaDashStates.cpp` の `cursor_`)で、
      **クリック可能な UI ボタンがゲーム側に 1 つも無い**。エンジンのポインタ経路は
      通るようになったので、`UIButtonComponent` を使う画面を作れば動くはず。
      **ゲームが UI ボタンを持ったときに見直す。**
- [ ] ImGui のウィンドウ上を触っても、仮想パッドが誤反応しない — **未対応(既知の限界)**。下記

**既知の限界: 逆方向の取り合いは未実装**

仮想パッドは生のタッチを円で判定するだけで ImGui の存在を知らないため、**ImGui ウィンドウを
パッドの円の上へ動かして触ると、パッドも反応する**。ImGui ウィンドウは既定で左上、パッドの円は
左下・右下なので通常は重ならず、当面はデバッグ UI を避けて置けば済む。
直すなら「ImGui が前フレームにポインタを捕らえていたら、仮想パッドは**新規の掴み**を行わない
(掴み中の指は継続)」という形になる。`SuppressMouse` と同じ 1 フレーム遅れの仕組み。

**P7 で分かったこと**

1. **`aq::hid` の自由関数は ImGui シムから使えない。** `IsMousePressed` 等は先頭で
   `IsMouseSuppressed()` を見て false を返す。抑制は「ImGui が使っている間ゲーム側を止める」
   ための仕組みなので、シムがこれを使うと **ImGui が入力を取った次のフレームから何も届かず、
   永久に触れなくなる**。`InputManager::GetMousePtr()` から `Mouse` を取り、抑制を見ない
   生の判定を読むこと。
2. **入力の更新順が効く。** マウスをパッドより先に更新すると、仮想パッドが指を掴む前に
   ポインタが拾ってしまい、スティックを倒した指で裏の UI をクリックする。
   「タッチ → キーボード → パッド → `pointerTouch_` 構築 → マウス」の順に変えた。
3. **`adb input tap` の座標はアプリの座標と違う。** 画面座標で指定するが、アプリが受け取るのは
   ウィンドウ座標で、この端末ではノッチぶん **118px ずれる**(画面 2400 に対しアプリは 2282)。
   座標を指定した検証では差し引きが要る。
4. **スケールだけでは足りず、入力だけでも足りない。** 拡大しないと指では押せず、
   入力を繋がないと何も起きない。両方そろって初めて実用になる。
