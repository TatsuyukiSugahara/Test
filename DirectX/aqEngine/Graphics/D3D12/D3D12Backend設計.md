...
|---|---|---|
| [0] CBV b0 | VS 定数（world/view/proj）/ UI b0（CircleGauge） | `VSSetConstantBuffer(0)` / `PSSetConstantBuffer(0)` |
| [1] CBV b1 | UI b1（SdfText） | `PSSetConstantBuffer(1)` |
| [2] DescriptorTable SRV t0..tN | テクスチャ | `PSSetShaderResource` |
| [3] DescriptorTable Sampler s0 | サンプラー | `PsSetSampler` |

> b0 が VS と PS で意味が異なる箇所がある（UI は PS で b0 を使う）。**可視性 `D3D12_SHADER_VISIBILITY_ALL` で 1 本に統合**するか、用途別に複数ルートシグネチャを持つか要検討（§13）。まずは ALL 可視の単一ルートシグネチャで開始する。
...
| **P2** | テクスチャ＋サンプラー（SRVディスクリプタヒープ／テーブル）＋ディスクリプタリング | テクスチャ付きメッシュ・**UI** が出る | ⏳ 未着手 |
| **P3** | オフスクリーンRT＋深度＋デプスモード／ブレンド／シザー＋ステート追跡 | Deferred/ポストプロセス相当 | ⏳ 未着手 |
| **P4** | コンピュート（Dispatch/UAV/SRV）＋シャドウ（slice DSV）＋imgui dx12 | フルパイプライン | ⏳ 未着手 |

各フェーズ後に DX11/DX12 を切替ビルドして回帰確認。

> **現状の制約**: P1b まででバックエンドは「コンパイル・リンク成立」状態。描画記録パス（PSO解決・ルートCBVバインド・VBV/IBV・DrawIndexed）は実装済みだが、**実機 GPU での描画検証は未実施**。フルシーンの実描画には P2（テクスチャ／SRVヒープ）・P3（オフスクリーンRT／深度）が必要。`ENGINE_GRAPHICS_D3D12` 有効化時、テクスチャ生成（`CreateTexture2D`）・深度（`CreateDepthMap`）・オフスクリーンRT（`CreateOffscreenRenderTarget`）は未実装のため、それらを使う初期化箇所で停止する。クリア画面（`Present`）単体は P0 で動作する。
>
