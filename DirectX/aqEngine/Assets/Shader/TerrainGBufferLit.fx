...
// TerrainLit 用 G-Buffer パス (MRT 書き込み)。
// スプラットマップブレンドでアルベドを生成し、G-Buffer に出力する。
// VSMain は TerrainLit.fx と同一レイアウト。
...
// TerrainLit.fx と同一の no-tile サンプリングヘルパー
...
{
    float  tiling  = max(params[0].x, 0.001);
    ...
    PSOutput o;
    o.gbuffer0 = float4(albedo, 1.0);                          // specMask = 1
