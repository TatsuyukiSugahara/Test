...
- (P8 で改善) 箱は `StaticMesh::SetLocalBounds` の明示 AABB でフラスタムカリング対象になった
  (カリング設計.md §13)。中盤以降 99〜118fps・リザルト 224fps まで回復したが、
  **開始地点はコースの大半(並行ストレート+ループ+カーブ)が視錐台内に入るため 66fps** で
  180fps 復帰には未達。次の一手はタイルのインスタンシング化(per-instance カリング)か Hi-Z 適用。
