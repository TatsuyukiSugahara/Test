...
> 対象コミット: 7036ee5 / 最終更新: 2026-09-11
...
2. ~~**起動時のシェーダコンパイルが起動時間の大半を占める**~~ → **対処不要と判明(2026-09-11 に再測定)**。
   **macOS が `newLibraryWithSource:` の結果をアプリ別にディスクキャッシュしている**
   (`$DARWIN_USER_CACHE_DIR/com.aqengine.aquadash/com.apple.metalfe`、実測 2.6MB)。

   | 状態 | 結果 |
   |---|---|
   | キャッシュ有効(常態) | 25 本 / **4.4 ms** / 50ms 超 **0 本** |
   | キャッシュキーを変えた直後 | 25 本 / **895 ms** / 50ms 超 10 本 |

   共有キャッシュを消す代わりに**生成 `.metal` にコメントを 1 行足してキャッシュキーだけ変える**
   方法で切り分けた(確認後に除去済み)。**`MTLBinaryArchive` を自前で持つ必要は無い**し、
   Metal Toolchain を入れて `.metallib` を事前ビルドする動機もこの目的では消えた。
   **900ms が出るのはシェーダを変更した直後の 1 回だけ。**

   **P0.5 で記録した「812ms / 起動の 7 割超」はコールドキャッシュの値だった。**
   測り直さずに最適化へ進んでいたら、不要な作業を 1 本まるごとやっていた。
   以下は当時の記録:
