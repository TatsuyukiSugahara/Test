...
// ディファードライティングパス: GBuffer を読み取りライティング結果をシーン RT に書き込む。
// SV_VertexID でフルスクリーントライアングルを生成する（頂点バッファ不要）。
...
Texture2D gbuffer0 : register(t8);   // albedo.rgb + specMask
Texture2D gbuffer1 : register(t9);   // N.xyz + gloss
Texture2D gbuffer2 : register(t10);  // worldPos.xyz + specularIntensity
