...
- **オフスクリーンパス(512²)がディファード経路でシーンを描けない**(2026-09-06 発見)。
  ライティング/フォワードが `OMSetRenderTargetWithDepth(オフスクリーンRT, GBuffer0)` で
  512² RTV と 1920×1080 DSV を同時バインドしており寸法不一致(D3D の制約違反、静かに失敗)。
  対処案: オフスクリーン RT に自前深度を持たせるか、オフスクリーン用に縮小 GBuffer を用意する。
  AquaDash のミニマップはこれを避けて UI 点列方式で実装した。

- **`BloomRenderer` の名前が実態と合っていない**(2026-09-06)。カメラモーションブラーを前段に組み込んだため、
  実態は「ポストプロセスチェーン(モーションブラー → Bloom → トーンマップ)」。`Renderer` のポストプロセス
  差し込み口が単一スロットのための妥協。対処案: ① `PostProcessChainRenderer` 等へリネーム、または
  ② `MotionBlurRenderer` を独立させ順序付きチェーン機構を `Renderer` に追加(本筋)。後から対処する。
