...
- **(未修正 2026-09-12 検出)タイトル画面で放置すると `MetalConstantBuffer` のスライスが
  増え続け、約 28 秒で上限 8192KB に達して「リングが枯渇しました。最終スライスを
  使い回します。描画が最後の値で潰れます」が出て画面が真っ黒になる。**
  リングが毎フレーム回収されていない。P20 の実機確認中に踏んだが草とは無関係
  (タイトル画面にはステージのエンティティが無い)。長時間プレイで描画が壊れるので
  エンジン側で別途対応が必要。
