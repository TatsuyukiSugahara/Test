...
| ライフサイクル(P5a) | ホーム → 復帰を **11 往復して落ちない**。背面では**サウンドのコールバックが完全に止まり**(10 秒間ログ無し)、復帰で**フレームカウンタが連続したまま**再開する(二重再生なし)。復帰後もタッチが生きている |
...
**実機の入手時期が読めないため、P5 を 2 つに割った**
([Android移植設計.md](Android移植設計.md) の P5a / P5b と同じ流儀)。
**P5a はシミュレータだけで完結する**ので先に進める。

#### P5a: ライフサイクル(シミュレータで完結)

- `PlatformiOS::IsRenderable()` を背面で `false` に(§3.4)
- `IPlatform::OnSuspend` / `OnResume` の実装と `CADisplayLink` の停止/再開
- `AqAppDelegate` の `applicationDidEnterBackground:` / `applicationWillEnterForeground:` から呼ぶ

> **停止の順序に注意。**`CADisplayLink` を先に止めると `Engine::FrameStep` が
> 呼ばれなくなり、`SyncSoundActivity()` が走らないので**サウンドが止まらないまま**になる。
> 「`IsRenderable` を false にする → **フレームを 1 回だけ回す** → リンクを止める」の順にする。
> この 1 回で `SyncSoundActivity` がサウンドを止め、描画は `IsRenderable` が false なので
> スキップされる(GPU は触らない)。
>
> **フォーカス喪失では止めない。**`applicationWillResignActive`(通知シェード・
> アプリスイッチャ)ではなく **`applicationDidEnterBackground`** を使う。
> Android が「通知シェードを下ろしただけでは窓は生きているので止めない」としているのと同じ判断。

**評価チェックリスト** — 2026-09-14 実施

- [x] ホームに戻る → 復帰、を **11 回**繰り返して落ちない(`suspended` / `resumed` が各 11 回、
      クラッシュ痕跡 0、プロセスは生存)
- [x] バックグラウンド中に Metal のコマンドを出していない。
      `CADisplayLink` を `paused` にしているのでフレーム自体が回らない
- [x] 背面で **BGM が止まり**、復帰で**続きから鳴る**(P4 で確認できなかった項目)。
      サウンドのコールバックが **10 秒間まったく呼ばれず**、復帰後は
      **フレームカウンタが 48128 きっかり増えて連続**(1106944 → 1155072)。
      **飛びも重なりも無い = ミキサの読み位置が凍っていた**ことの裏取りになる
- [x] 復帰後にタッチ操作が生き続けている(11 往復の後にタップでタイトル → ステージへ)
- [x] **Mac の回帰**: Metal / Vulkan とも Release がビルドでき、
      ステージ走行(60.1 FPS / 417 km/h)とサウンド開始を確認
- [x] iOS 実機構成(`ios-xcode`)のコンパイルが通る
- [ ] **Windows 3 構成の回帰 — 未確認**(この環境ではビルドできない)。
      変更は **iOS 専用の 3 ファイルのみ**で、共通コード(`IPlatform` / `Engine`)には
      一切触れていない

**P5a で分かったこと**

- **停止の順序が設計どおり効いた。** `CADisplayLink` を止める前に**フレームを 1 回だけ回す**
  ことで `Engine::SyncSoundActivity()` が走り、サウンドが止まる。
  この 1 回は `IsRenderable()` が false なので描画はスキップされ、**GPU は触らない**。
  順序を逆にすると背面で BGM が鳴り続ける(P4 で実際にそうなっていた)。
- **`applicationWillResignActive` ではなく `applicationDidEnterBackground` を使う。**
  前者は通知シェード・アプリスイッチャ・着信でも飛んでくるが、そのときはまだ前面にいる。
  Android が「通知シェードを下ろしただけでは窓は生きているので止めない」としているのと同じ判断。
- **`invalidate` ではなく `paused`。** 復帰のたびに `CADisplayLink` を張り直さずに済む。
  破棄は `StopFrameLoop`(終了時)の仕事として分けてある。

#### P5b: 実機(実機が用意でき次第)

**ここで初めて実機が必要**(§0.5-2)。**P1〜P5a で「シミュレータでは確認できない」として
送った項目がすべてここに集まっている。**
