...
> 対象コミット: 655031f / 最終更新: 2026-09-07
...
- (P8/P9 で改善) 箱は `StaticMesh::SetLocalBounds` の明示 AABB でフラスタムカリング対象になり
  (カリング設計.md §13)、P9 で路面タイルを 1 ドローのインスタンス描画 + per-instance
  フラスタムカリングへ移行(同 §14)。開始地点 66→97fps・リザルト 277fps。
  **180fps 復帰にはまだ未達**で、残り約 10ms の有力候補は視錐台内のコイン個別ドロー
  (最大 186 枚が 1 枚 1 ドロー)。次の一手はコインのインスタンス化
  (取得/復活は InstancePoints の増減で表現)。
